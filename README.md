...
- 💬 **切换账号保留聊天历史**：切换账号时自动备份并恢复之前的 AI 对话记录
- 🔄 **无缝体验**：重启后继续之前的对话，无需重新开始

### v2.8.11 🎯 分模型阈值 & 体验优化

- 🎚️ **分模型切换阈值**：可为 Claude / Gemini Pro / Gemini Flash 分别设置切换阈值（0% 表示不检查该模型）
- 📥📤 **图标优化**：导入导出图标方向更直观（⬆️导入 / ⬇️导出）
- 🔄 **批量导入跳过重复**：导入重复账号时自动跳过，不再报错
- 🐛 **修复弹窗双按钮**：修复自动切换弹窗出现两个"取消"按钮的问题

### v2.8.0 ✨ 用户体验优化

- 🚫 **修复重复弹窗问题**：点击"取消"后不再重复提示切换该模型
- ⚙️ **设置面板**：可在账号管理侧边栏直接调整自动切换阈值
- 🎚️ **可视化滑块**：拖动滑块即可调整阈值（0%-100%）
- ⚡ **智能触发**：配额显著下降（≥5%）时立即检查，无需等待
- 🔄 **实时刷新**：自动切换检查前会刷新所有账号配额
